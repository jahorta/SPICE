...
| 3 | `s044.sml` / `s044.sst` | High-value uninspected candidate with type `8`, `9`, and rare type `10` in the same stage. Good candidate for seeing whether type `10` controls a visually obvious effect in a richer scene. | `22` records; command counts `{0:22, 1:1, 8:10, 9:10, 10:1}`; model links `21/21`; decoded SML size `2501128`. |
