...
public:
    std::optional<spice::modeling::Structs::Color> diffuse;
    std::optional<spice::modeling::Structs::Color> ambient;
    std::optional<spice::modeling::Structs::Color> specular;
    std::uint8_t specular_exponent = 0;

    MaterialChunk() : SizedPolyChunk(PolyChunkType::Material_Diffuse) {
        diffuse = spice::modeling::Structs::Color::white();
    }
    ...
    [[nodiscard]] static MaterialChunk read(const spice::modeling::Structs::EndianStackReader& reader,
        ...
        if ((typeValue & 0x01u) != 0) {
            result.diffuse = spice::modeling::Structs::EndianIOExtensions::read_color(
                reader, address, spice::modeling::Structs::ColorIOType::ARGB8_16);
        }

        if ((typeValue & 0x02u) != 0) {
            result.ambient = spice::modeling::Structs::EndianIOExtensions::read_color(
                reader, address, spice::modeling::Structs::ColorIOType::ARGB8_16);
        }

        if ((typeValue & 0x04u) != 0) {
            auto spec = spice::modeling::Structs::EndianIOExtensions::read_color(
                reader, address, spice::modeling::Structs::ColorIOType::ARGB8_16);
