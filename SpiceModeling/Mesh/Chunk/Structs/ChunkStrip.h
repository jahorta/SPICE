...
struct ChunkStrip {
    ...
    [[nodiscard]] static ChunkStrip read(const spice::modeling::Structs::EndianStackReader& reader,
        ...
        for (std::size_t i = 0; i < result.corners.size(); ++i) {
            ...
            if (hasUv) {
                corner.texcoord = spice::modeling::Structs::EndianIOExtensions::read_vector2(
                    reader, address, spice::modeling::Structs::FloatIOType::Short) * uvMultiplier;
                if (hasUv2) {
                    corner.texcoord2 = spice::modeling::Structs::EndianIOExtensions::read_vector2(
                        reader, address, spice::modeling::Structs::FloatIOType::Short) * uvMultiplier;
                }
            }

            if (hasNormal) {
                corner.normal = spice::modeling::Structs::EndianIOExtensions::read_vector3(
                    reader, address, spice::modeling::Structs::FloatIOType::Short) * normalFactor;
            } else if (hasColor) {
                corner.color = spice::modeling::Structs::EndianIOExtensions::read_color(
                    reader, address, spice::modeling::Structs::ColorIOType::ARGB8_16);
            }
