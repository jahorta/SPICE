...
public:
    ...
    void recalculate_bounds_from_vertices() {
        ...
        std::vector<spice::modeling::Structs::Vector3> points;
        for (const auto& chunk : vertex_chunks) {
            if (!chunk.has_value()) {
                continue;
            }
        ...
        mesh_bounds = points.empty() ? spice::modeling::Structs::Bounds{} : spice::modeling::Structs::Bounds::from_points(points);
    }

    [[nodiscard]] static std::shared_ptr<ChunkAttach> read(const spice::modeling::Structs::EndianStackReader& reader,
        ...
        result->mesh_bounds = spice::modeling::Structs::Bounds::read(reader, address + 8u);
        return result;
    }
