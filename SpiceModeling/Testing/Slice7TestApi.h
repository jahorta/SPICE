...
namespace spice::modeling::Testing::Slice7 {

using Motion = ::spice::modeling::Animation::Motion;

[[nodiscard]] inline bool CheckIsAnimationBlock(std::span<const std::byte> data) {
    const auto scan = ::spice::modeling::File::NJBlockUtility::ScanBlocks(data);
    for (const auto& block : scan.blocks) {
        if (block.role == ::spice::modeling::File::NJBlockRole::Animation) {
            return true;
        }
    }
    return false;
...
[[nodiscard]] inline Motion ReadMotionBlock(std::span<const std::byte> data,
    ...
    for (const auto& block : scan.blocks) {
        ...
        const ::spice::modeling::Structs::EndianStackReader reader(data, scan.size_endian);
        return ::spice::modeling::Animation::Motion::read(reader, dataAddress, nodeCount, imageBase, shortRot);
    }
