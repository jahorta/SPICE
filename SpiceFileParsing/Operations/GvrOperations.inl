...
spice::gvm::encoding::EncodeOptions buildCreateGvrEncodeOptions(
    const spice::fileparsing::GvrEncodingSettings& settings) {
    ...
    if (settings.globalIndex.kind == spice::fileparsing::GvrGlobalIndexKind::Value) {
...
std::size_t createGvrBatch(
    ...
    for (const auto& entry : std::filesystem::directory_iterator(operation.paths.input)) {
        ...
        emit(context, spice::fileparsing::EventLevel::Progress,
            "[SpiceFileParsing]   - Creating GVR: ", entry.path().filename().string());
...
std::size_t replaceGvrBatch(
    ...
    for (const auto& entry : std::filesystem::directory_iterator(operation.paths.input)) {
        ...
        emit(context, spice::fileparsing::EventLevel::Progress,
            "[SpiceFileParsing]   - Replacing GVR: ", entry.path().filename().string());
