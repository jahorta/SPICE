...
std::size_t selectMldTextureIndex(
    const spice::fileparsing::MldTextureSelector& selector,
    const spice::mld::model::MldTextureArchive& archive) {
    if (const auto* index = std::get_if<spice::fileparsing::TextureIndex>(&selector)) {
    ...
    const auto& name = std::get<spice::fileparsing::TextureName>(selector).value;
...
void replaceMldTextureFromPngFile(
    const spice::fileparsing::ReplaceMldTextureRequest& request) {
...
void extractMldTextureToGvrFile(
    const spice::fileparsing::ExtractMldTextureGvrRequest& request) {
...
void extractMldTextureToPngFile(
    const spice::fileparsing::ExtractMldTexturePngRequest& request) {
    const auto& sourceMldPath = request.input;
    const auto& outputPath = request.output;
    const auto extracted = extractMldTexture(request.selector, sourceMldPath);
...
std::optional<std::filesystem::path> maybeInvokeDotnetBridge(
    ...
    if (!std::filesystem::exists(bridgePath)) {
        emit(context, spice::fileparsing::EventLevel::Warning,
            "[SpiceFileParsing] WARNING: .NET bridge executable does not exist: ", bridgePath.string());
        return std::nullopt;
    }
    ...
    if (exitCode != 0) {
        emit(context, spice::fileparsing::EventLevel::Warning,
            "[SpiceFileParsing] WARNING: .NET bridge run failed with exit code ",
    ...
    if (!std::filesystem::exists(bridgeOutPath)) {
        emit(context, spice::fileparsing::EventLevel::Warning,
            "[SpiceFileParsing] WARNING: .NET bridge did not emit expected output: ", bridgeOutPath.string());
        return std::nullopt;
    }
...
void writeExtractedSpatialBlocks(
    ...
    for (std::size_t i = 0; i < blocks.size(); ++i) {
        ...
        if (!writeAllBytes(blockPath, std::span<const std::uint8_t>(block.bytes.data(), block.bytes.size()))) {
            emit(context, spice::fileparsing::EventLevel::Warning,
                "[SpiceFileParsing] WARNING: failed to write extracted GRND/GOBJ block: ", blockPath.string());
